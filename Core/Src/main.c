...
int main(void)
{
  ...
  while (1)
  {
    ...
    switch(getSn_SR(HTTP_SOCKET))
    {
      ...
      {
        recv(HTTP_SOCKET, req, size);
        size = getResHttp(req, res, &sensor_data);
        send(HTTP_SOCKET, res, size);
        close(HTTP_SOCKET);
      }
      break;
      ...
      case SOCK_CLOSED:
      socket(HTTP_SOCKET, Sn_MR_TCP, HTTP_PORT, 0x00);
      break;
